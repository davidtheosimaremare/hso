...
import { AlertTriangle, Calendar, Building2, User, FileText, ArrowLeft, Loader2, Package, Truck, MapPin } from 'lucide-vue-next'
...
const fetchDetail = async () => {
    ...
    try {
        ...
        if (!header) throw new Error("Delivery Order tidak ditemukan.")
  ...
  <div class="min-h-screen bg-gray-50/50 dark:bg-[#0f172a] pb-20 font-sans transition-colors duration-300">
    <div class="pt-6 space-y-6">
      ...
      <div v-if="errorMessage" class="flex flex-col items-center justify-center p-12 text-center bg-white dark:bg-slate-800 rounded-xl border border-red-100 dark:border-red-900 shadow-sm animate-in zoom-in-95 duration-300">
        ...
        <Button @click="router.push('/delivery-orders')" variant="outline">Kembali</Button>
      </div>
      ...
      <div v-else-if="isLoading" class="flex flex-col items-center justify-center h-[60vh]">
        ...
        <p class="text-slate-500 mt-4 text-sm font-medium">Memuat detail DO...</p>
      </div>

      <div v-else-if="doDetail" class="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
        
        <!-- HEADER -->
        <div class="flex flex-col gap-4">
            <Button @click="router.push('/delivery-orders')" variant="ghost" class="w-fit pl-0 hover:bg-transparent hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <div class="space-y-3">
                    <Badge variant="outline" class="uppercase tracking-widest text-[10px] px-2 py-0.5 border-slate-300 text-slate-500">Delivery Order</Badge>
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500 dark:text-gray-400">
                        <span class="flex items-center gap-1.5"><Calendar class="w-4 h-4"/> {{ formatDate(doDetail.trans_date) }}</span>
                        <span class="hidden md:inline w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></span>
                        <span class="flex items-center gap-1.5"><User class="w-4 h-4"/> {{ doDetail.customer_name || 'No Customer' }}</span>
                    </div>
                </div>
                
                <div class="flex flex-col items-end gap-2">
                     <Badge class="px-3 py-1 text-sm font-bold uppercase transition-all" :class="getStatusColor(doDetail.status_name)">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card class="border shadow-sm rounded-xl bg-white dark:bg-slate-800 dark:border-slate-700">
                    <CardHeader class="pb-3"><CardTitle class="text-sm font-bold uppercase text-slate-500 dark:text-slate-400 flex items-center gap-2"><MapPin class="w-4 h-4"/> Alamat Kirim</CardTitle></CardHeader>
                    <CardContent>
                        <p class="text-slate-900 dark:text-white font-medium whitespace-pre-wrap">{{ doDetail.ship_to || 'Tidak ada alamat' }}</p>
                    </CardContent>
                </Card>
                <Card class="border shadow-sm rounded-xl bg-white dark:bg-slate-800 dark:border-slate-700">
                    <CardHeader class="pb-3"><CardTitle class="text-sm font-bold uppercase text-slate-500 dark:text-slate-400 flex items-center gap-2"><Truck class="w-4 h-4"/> Info Pengiriman</CardTitle></CardHeader>
                    <CardContent>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <p class="text-xs text-slate-500 mb-1">Driver</p>
                                <p class="font-bold text-slate-900 dark:text-white">{{ doDetail.driver_name || '-' }}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
        ...
        <Card class="border shadow-sm rounded-xl overflow-hidden bg-white dark:bg-slate-800 dark:border-slate-700">
            <CardHeader class="flex flex-row items-center justify-between border-b border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800">
                ...
                <Badge variant="secondary">{{ doDetail.items.length }} Items</Badge>
            </CardHeader>
            <CardContent class="p-0">
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="(item, index) in doDetail.items" :key="item.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors border-b border-slate-100 dark:border-slate-700 last:border-0">
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <div v-if="item.hso_number" class="mt-1">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <span class="cursor-pointer text-[10px] text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 font-medium ml-1 underline" @click.stop="router.push(`/sales-orders/${item.hso_number.replace(/\//g, '-')}?search=${item.item_code}&highlight=${item.item_code}`)">
                                                (Lihat HSO)
                                            </span>
                                        </div>
                                    </div>
